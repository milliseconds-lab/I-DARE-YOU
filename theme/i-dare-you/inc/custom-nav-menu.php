...
class Nav_Menu_Icon extends Walker_Nav_Menu {
    function start_el(&$output, $item, $depth, $args) {
        global $wp_query;
        $indent = ($depth)? str_repeat('\t', $depth):'';
        $class_names = $value = '';
        ...
        $item_output .= '<span class="icon">'.$item->custom.'</span>';
        $item_output .= '<span class="text">'.$args->link_before.apply_filters('the_title', $item->title, $item->ID).$args->link_after.'</span>';
        $item_output .= '</a>';
